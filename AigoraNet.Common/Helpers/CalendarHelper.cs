...
public class CalendarHelper
{
    ...
    public List<List<DateTime>> GetWeeks()
    {
        var result = new List<List<DateTime>>();

        result.Add(this.List.Take(7).ToList());
        result.Add(this.List.Skip(7).Take(7).ToList());
        result.Add(this.List.Skip(14).Take(7).ToList());
        result.Add(this.List.Skip(21).Take(7).ToList());
        if (this.List.Count > 28)
        {
            result.Add(this.List.Skip(28).Take(7).ToList());
        }
        if (this.List.Count > 35)
        {
            result.Add(this.List.Skip(35).Take(7).ToList());
        }
        if (this.List.Count > 42)
        {
            result.Add(this.List.Skip(42).Take(7).ToList());
        }
